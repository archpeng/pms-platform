...
- `ai-pms`: PMS workflow orchestration, dry-run/confirm identity law, pending/callback/outbox state, projection coordination, and typed endpoint contracts used by `ai-conversation`.
...
- does not own `ai-pms` workflow orchestration, pending state, callback state, outbox state, or projection coordination;
- does not expose arbitrary HTTP, shell, file, SQL, Base/Bitable, or generic customer-chat tools.

## Dependency boundary

Do not add `@mariozechner/pi-agent-core`, `@mariozechner/pi-ai`, `@mariozechner/pi-coding-agent`, or Feishu SDK packages such as `@larksuiteoapi/node-sdk` to this repo. Pi/LLM conversation runtime belongs in `ai-conversation`; Feishu runtime belongs in `adapter-feishu`; workflow orchestration belongs in `ai-pms`.
