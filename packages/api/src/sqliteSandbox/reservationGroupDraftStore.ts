...
export abstract class SqliteSandboxReservationGroupDraftStore extends SqliteSandboxReservationDraftStore {
  createReservationGroupDraft(
    request: ReservationGroupDraftCreateApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    return this.runInTransaction(() =>
      this.createReservationGroupDraftRecord(request),
    );
  }

  updateReservationGroupDraft(
    request: ReservationGroupDraftUpdateApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    return this.runInTransaction(() =>
      this.updateReservationGroupDraftRecord(request),
    );
  }

  quoteReservationGroupDraft(
    request: ReservationGroupQuoteApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    return this.runInTransaction(() =>
      this.quoteReservationGroupDraftRecord(request),
    );
  }

  prepareConfirmReservationGroupDraft(
    request: ReservationGroupPrepareConfirmApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    return this.runInTransaction(() =>
      this.prepareConfirmReservationGroupDraftRecord(request),
    );
  }

  cancelReservationGroupDraft(
    request: ReservationGroupDraftCancelApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    return this.runInTransaction(() =>
      this.cancelReservationGroupDraftRecord(request),
    );
  }

  protected createReservationGroupDraftRecord(
    request: ReservationGroupDraftCreateApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    ...
    const status = groupDraftStatusFromMissingSlots(
      missingSlots,
      expiresAt,
      createdAt,
    );
    ...
    const response = reservationGroupDraftSuccessResponse(
      request.operation,
      'created',
      groupDraft,
      [auditRef],
  ...
  protected updateReservationGroupDraftRecord(
    request: ReservationGroupDraftUpdateApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    const replay = this.reservationGroupDraftReplayOrConflict(request);
    if (replay) return replay;
    ...
    const missingSlots = cloneValue(
      request.missingSlots ?? deriveGroupMissingSlots(slots),
    );
    const status =
  ...
  protected quoteReservationGroupDraftRecord(
    request: ReservationGroupQuoteApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    const replay = this.reservationGroupDraftReplayOrConflict(request);
    if (replay) return replay;
    ...
    const inactive = reservationGroupDraftInactiveResponse(
      request,
      existing,
      quotedAt,
    );
    ...
    if (existing.missingSlots.length > 0)
      return reservationGroupDraftMissingSlotsResponse(request, existing);
  ...
  protected prepareConfirmReservationGroupDraftRecord(
    request: ReservationGroupPrepareConfirmApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    const replay = this.reservationGroupDraftReplayOrConflict(request);
    if (replay) return replay;
    ...
    if (existing.missingSlots.length > 0)
      return reservationGroupDraftMissingSlotsResponse(request, existing);
    if (!existing.quote)
      return reservationGroupDraftQuoteRequiredResponse(request, existing);
    if (request.quoteRef && request.quoteRef !== existing.quote.quoteRef)
      return reservationGroupDraftQuoteMismatchResponse(request, existing);

    const pendingAction = reservationGroupDraftPendingAction(
      existing,
  ...
  protected cancelReservationGroupDraftRecord(
    request: ReservationGroupDraftCancelApiRequest,
  ): ReservationGroupDraftWorkflowApiResponse {
    const replay = this.reservationGroupDraftReplayOrConflict(request);
    if (replay) return replay;
  ...
  protected reservationGroupDraftReplayOrConflict(
    request:
    ...
    return cloneValue(
      existing.response,
    ) as ReservationGroupDraftWorkflowApiResponse;
  }
}
