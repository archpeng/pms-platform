...
`projectionKind=reservationWorkflow` is projected by `pms-platform` through adapter operation `pms_base_upsert_operation_request` into `PMS操作请求`, not into `预订`. Single-room confirmed reservation drafts now also materialize a PMS-owned `ReservationReadModel`; `projectionKind=reservation` is projected through `pms_base_upsert_reservation_projection` into `预订`. Group workflow events remain operator-visible operation workflow rows until a group reservation materialization contract exists.
